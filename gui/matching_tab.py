...
class MatchingTab(QWidget):
    ...
    def display_matching_results(self, results):
        ...
        # Check if we have refined phases to prioritize
        self._apply_refined_phase_priorities()
    ...
    def _apply_refined_phase_priorities(self):
        ...
        if not hasattr(self, 'multi_phase_analyzer'):
            return
            
        # Update candidate phases with any cached refined parameters
        if self.reference_phases:
            updated_phases = self.multi_phase_analyzer.update_candidate_phases_with_refinement(
                self.reference_phases
            )
            
            if len(updated_phases) != len(self.reference_phases):
                print(f"Updated {len(updated_phases)} phases with refined parameters")
                self.reference_phases = updated_phases
                
                # Re-run matching with updated phases if we have results
                if self.matching_results:
                    print("Re-running matching with refined phase parameters...")
                    self.start_matching()
